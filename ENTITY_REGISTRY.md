...
| Discovery Session | `discovery_sessions` | `routers/discovery.py` | `SessionCreate` | `DiscoveryAdmin`, `DiscoveryClient` |
| Discovery Answer | `discovery_answers` | `routers/discovery.py` | `AnswerPayload` | `QuestionField`, `DiscoverySummary` |
| Discovery Attachment | `discovery_attachments` | `routers/discovery.py` | `AttachmentMeta` | `AttachmentUploader` |
...
Component:   DiscoveryAdmin.jsx, DiscoveryClient.jsx
Key Fields:
  id (UUID4 = share token), client_name, project_name, contact_person,
  contact_email, notes, status (draft|submitted|archived),
  created_at, updated_at, submitted_at

⚠️ id berperan sebagai token publik di URL (/discovery/{id}). UUID4 unguessable.
⚠️ status submitted/archived → answers tidak bisa diubah (read-only).
```

### discovery_answers
```
Collection:  discovery_answers
Router:      routers/discovery.py
Component:   QuestionField.jsx, DiscoverySummary.jsx
Key Fields:
  session_id, question_id, value, skipped (bool),
  other_text (isian "Lainnya"), note, updated_at

⚠️ Unique per (session_id, question_id) — upsert, bukan insert ganda.
⚠️ question_id wajib valid (dari get_all_question_ids()).
```

### discovery_attachments
```
Collection:  discovery_attachments
Router:      routers/discovery.py
Component:   AttachmentUploader.jsx
Key Fields:
  id, session_id, question_id, original_filename, content_type, size,
  stored_filename (INTERNAL), full_path (INTERNAL), uploaded_at

⚠️ stored_filename & full_path TIDAK pernah dikirim ke client (di-exclude di query).
⚠️ File fisik disimpan di /app/uploads/discovery/<session_id>/.
```
...
sales_returns          Prefix: sret_  — retur/tukar/Barang Sisa (BS) cacat + dampak stok
special_orders         Prefix: sord_  — Special Order (SKU belum ada → MD + Purchasing) + estimasi
